...
import { postChangePasswordAPI, postSendCodeAPI } from '@/services/login';
// 存储用户信息
...
const handleLogin = async () => {
  const res = await postChangePasswordAPI(loginData.value);
  ...
  <view class="viewport">
    ...
    <view class="login">
      ...
      <input
        ...
        class="phoneInput password"
        type="password"
        placeholder="请输入新密码"
        placeholder-class="input-placeholder"
      />
      <button class="button phone" @click="handleLogin">修改</button>
    </view>
  </view>
</template>
...
.password {
  background: url(@/static/image/password.png) no-repeat 0 0;
